...
const ImgUpload = ({ label, value, onChange, testid }) => {
  ...
  const src = value ? (value.startsWith("http") || value.startsWith("/") ? value : fileUrl(value)) : null;
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <input type="text" value={value || ""} onChange={(e) => onChange(e.target.value)} placeholder="Type GitHub link (e.g., /logo.png)" className="w-full max-w-xs bg-[#141414] border border-white/15 rounded-lg px-3 py-1.5 text-sm text-zinc-300 outline-none focus:border-[#FF3B30]" />
    </div>
  );
};

export default function Admin() {
  const { settings, refresh, TEAM_COLOR } = useSettings();
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pb-20">
        ...
        {tab === "League Config" && (
          <div className="space-y-6" data-testid="admin-league">
            ...
            <div className="card-tech rounded-xl p-6">
               <h3 className="font-display text-2xl text-[#FF3B30] mb-3">Tournament Format</h3>
               <select value={form.format || "Singles"} onChange={(e) => setForm({ ...form, format: e.target.value })} className="bg-[#0A0A0A] border border-white/15 rounded-lg px-4 py-2 text-white outline-none focus:border-[#FF3B30]">
                 <option value="Singles">Singles (Max 10 Teams)</option>
                 <option value="Doubles">Doubles (Max 5 Teams)</option>
               </select>
            </div>

            <div className="card-tech rounded-xl p-6 space-y-4">
              <div className="flex justify-between items-end">
                <h3 className="font-display text-2xl text-[#007AFF]">Manage Teams</h3>
                <span className="text-sm text-zinc-400">Current: {Array.isArray(form.teams) ? form.teams.length : 0} / {form.format === "Doubles" ? "5" : "10"}</span>
              </div>
              
              {Array.isArray(form.teams) && form.teams.map((t, i) => (
                <div key={i} className="flex gap-3">
                  <input value={t?.name || ""} onChange={(e) => { const c = [...form.teams]; c[i].name = e.target.value; setForm({ ...form, teams: c }); }} className="flex-1 bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 text-white" placeholder="Team Name" />
                  <input type="color" value={t?.color || "#FFFFFF"} onChange={(e) => { const c = [...form.teams]; c[i].color = e.target.value; setForm({ ...form, teams: c }); }} className="h-10 w-14 rounded-lg cursor-pointer bg-transparent border-0" />
                  <button onClick={() => { const c = form.teams.filter((_, x) => x !== i); setForm({ ...form, teams: c }); }} className="text-[#FF3B30]"><Trash2 size={18} /></button>
                </div>
              ))}
              
              <button 
                onClick={() => setForm({ ...form, teams: [...(Array.isArray(form.teams) ? form.teams : []), { name: "New Team", color: "#FFFFFF" }] })} 
                disabled={Array.isArray(form.teams) && form.teams.length >= (form.format === "Doubles" ? 5 : 10)}
                className="inline-flex items-center gap-2 text-sm px-4 py-2 rounded-full border border-white/15 hover:border-white/40 disabled:opacity-50">
                <Plus size={15} /> Add Team
              </button>
            </div>

            <div className="card-tech rounded-xl p-6">
               <h3 className="font-display text-2xl text-[#22C55E] mb-4">Matches Per Pair</h3>
               <div className="grid sm:grid-cols-3 gap-4">
                 <div>
                   <label className="text-sm text-zinc-400 block mb-1">Table Tennis (5-10)</label>
                   <input type="number" min="5" max="10" value={form.tt_matches || 5} onChange={(e) => setForm({ ...form, tt_matches: Number(e.target.value) })} className="w-full bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 text-xl font-display text-white" />
                 </div>
                 <div>
                   <label className="text-sm text-zinc-400 block mb-1">Lawn Tennis (3-5)</label>
                   <input type="number" min="3" max="5" value={form.lt_matches || 3} onChange={(e) => setForm({ ...form, lt_matches: Number(e.target.value) })} className="w-full bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 text-xl font-display text-white" />
                 </div>
                 <div>
                   <label className="text-sm text-zinc-400 block mb-1">Badminton (3-5)</label>
                   <input type="number" min="3" max="5" value={form.bt_matches || 3} onChange={(e) => setForm({ ...form, bt_matches: Number(e.target.value) })} className="w-full bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 text-xl font-display text-white" />
                 </div>
               </div>
            </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button onClick={async () => {
                if(!window.confirm("Warning: This will delete all current matches and generate a completely new schedule. Proceed?")) return;
                try {
                  const { data } = await api.post("/matches/generate");
                  toast.success(`Generated ${data.total_matches} new fixtures!`);
                } catch { toast.error("Failed to generate fixtures"); }
              }} className="inline-flex items-center gap-2 px-5 py-2 rounded-full border border-[#FF3B30] text-[#FF3B30] font-semibold hover:bg-[#FF3B30]/10">
                Generate Fixtures
              </button>
            </div>
          </div>
        ...
        {tab === "Players" && (
          <div className="space-y-4" data-testid="admin-players">
            ...
            {players.map((p, i) => (
              <div key={p.id || i} className="card-tech rounded-xl p-5 grid sm:grid-cols-2 gap-3" data-testid={`admin-player-${p.number}`}>
                <div className="flex items-center gap-3">
                  <span className="font-display text-3xl w-8 text-center" style={{ color: (TEAM_COLOR && TEAM_COLOR[p.team]) ? TEAM_COLOR[p.team] : "#FFFFFF" }}>{p.number}</span>
                ...
                <input value={p.role || ""} onChange={(e) => { const c = [...players]; c[i].role = e.target.value; setPlayers(c); }} className="bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 text-white" placeholder="Role" />
                ...
                <textarea value={p.bio || ""} onChange={(e) => { const c = [...players]; c[i].bio = e.target.value; setPlayers(c); }} className="sm:col-span-2 bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 text-white" placeholder="Bio" rows={2} />
        ...
        {tab === "News" && (
          <div className="space-y-4" data-testid="admin-news">
            <div className="card-tech rounded-xl p-5 space-y-3">
              <input value={newNews.title} onChange={(e) => setNewNews({ ...newNews, title: e.target.value })} placeholder="Announcement title" className="w-full bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 text-white" data-testid="news-title" />
              <textarea value={newNews.body} onChange={(e) => setNewNews({ ...newNews, body: e.target.value })} placeholder="Details..." rows={3} className="w-full bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 text-white" data-testid="news-body" />
            ...
            {news.map((n) => (
              <div key={n.id} className="card-tech rounded-xl p-5 flex justify-between gap-4" data-testid={`news-item-${n.id}`}>
                <div><h4 className="font-semibold text-white">{n.title}</h4><p className="text-zinc-400 text-sm mt-1">{n.body}</p></div>
          ...
          <div className="space-y-4" data-testid="admin-history">
            {hist.map((h, i) => (
              <div key={i} className="card-tech rounded-xl p-5 space-y-2">
                <input value={h.year || ""} onChange={(e) => { const c = [...hist]; c[i].year = e.target.value; setHist(c); }} placeholder="Year" className="bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 w-32 text-white" />
                <textarea value={h.text || ""} onChange={(e) => { const c = [...hist]; c[i].text = e.target.value; setHist(c); }} placeholder="Milestone" rows={2} className="w-full bg-[#0A0A0A] border border-white/15 rounded-lg px-3 py-2 text-white" />
